...
* Aprender varios comandos útiles como tales:
...
## Requisitos previos
Antes de empezar esta tutorial, deberías poder localizar la [Terminal](https://es.wikipedia.org/wiki/Terminal_(OS_X)) de su compudatora o otra interfaz de línea de comandos. Esta es donde entrará y ejecutará los comandos de FFmpeg. Si necesita instrucción para acceder y usar la interfaz de línea de comandos, recomiendo la lección de Programming Historian [Introducción a la línea de comandos en Bash](https://programminghistorian.org/es/lecciones/introduccion-a-bash) para usarios de Mac y Linux o, para usarios de Windows, [Introducción a la línea de comandos de Windows con PowerShell](https://programminghistorian.org/es/lecciones/introduccion-a-powershell). Adicionalmente, una entendimiento básica de [códecss](https://es.wikipedia.org/wiki/C%C3%B3dec) y [contenedores](https://es.wikipedia.org/wiki/Formato_contenedor) audiovisuales también será útil para entender qué hace FFmpeg y cómo funciona. Proporcionaremos información adicional y discutiremos códecs y contenedores con más detalle en la sección Ejemplos de Comandos Preliminares de este tutorial.
...
[Linuxbrew](ttp://linuxbrew.sh/), una programa similar a Homebrew, se puede usar para instalar y mantener FFmepg en Linux. Reto Kromer también proporciona una guía útil, [La Guía de Instalación de Linux](https://avpres.net/FFmpeg/install_Linux.html=), que es similar a la instalación en Mac OS. Su distribución de Linux puede tener su [propio administrador de paquetes](https://www.linode.com/docs/tools-reference/linux-package-management/) que incluye paquets FFmpeg. Dependiendo en su distribución de Linux (Ubuntu, Fedora, Arch Linux, etc.) estas versiones pueden variar, así que usando Linuxbrew podría ser útil para asegurar la versión es la misma independientemente del tipo de Linux que esté utilizando.

## Otras Recursos para Instalar

* [Paquetas de descargar](https://www.ffmpeg.org/download.html)
  * FFmpeg permite el accesso a archivos binarios, código fuente, y versiones estáticos para Mac, Windows, y Linux directamente en el sitio web. Usarios pueden construir el marco sin una administrador de paquetes con estos recursos. Es probable que solo los usuarios avanzados quieran usar esta opción.
...
* Si ve algo como `-bash: ffmpeg: command not found`, algo ha ido mal.
  * Nota: Si esta usando una administrador de paquetes, es inprobable que encuentre este mensaje de error. Sin embargo, si hay un problema despues de instalar con una administrador de paquetes, es probable que haya un problema con la administrador de paquetes y no con FFmpeg. Consulte la solución de problemas por [Homebrew](https://docs.brew.sh/Troubleshooting), [Chocolatey](https://chocolatey.org/docs/troubleshooting), o [Linuxbrew](http://linuxbrew.sh/) para asegurar la administrador de paquetes esta funcionando correctamente en su computadora. Si esta intentando instalar sin una administrador de paquetes y ve este mensaje de error, haga una referencia cruzada su método con la La Guía Compilacion de FFmpeg anterior.
...
```bash
 ...
 ```
<div class="alert alert-warning">
Como con cualquier interfaz de línea de comandos, tendrá que escribir las rutas de archivos de los archivos entradas y salidas dependiendo en los lugares de los directorios de trabajar. En los ejemplos proporcionan en esta tutorial, las rutas de archivos no será escrito completamente y se supone que el usuario ha navegado al directorio de trabajo para ejecutar los comandos.</div>

A continuación, examinaremos algunos ejemplos de varios comandos diferentes que usan esta estructura y sintaxis. Adicionalmente, estos comandos demostrarán algunas de las características más útiles de FFmpeg y nos permiten familiarizarnos con la forma en que se construyen los archivos audiovisuales digitales.

# Para Empezar
Para esta tutorial, usaremos una película archivística se llama [*Destination Earth*](https://archive.org/details/4050_Destination_Earth_01_47_33_28) como nuestro objeto de estudio. Esta película ha sido puesta a disposición por el [Archivos Prelinger](https://es.wikipedia.org/wiki/Archivos_Prelinger) y el [Internet Archive](https://archive.org/). Estenada en 1956 y producido por [El American Petroleum Institute](https://es.wikipedia.org/wiki/American_Petroleum_Institute) y [John Sutherland Productions](https://en.wikipedia.org/wiki/John_Sutherland_(producer)), la película es un excelente ejemplo de la propaganda de la época Guerra Fria que exalta las virtudes del capitalismo y el estilo de vida americano. Utilizando el proceso de [Technicolor](https://es.wikipedia.org/wiki/Technicolor), este corto animado de ciencia ficción cuenta la historia de una sociedad marciana que vive bajo un gobierno opresivo y sus esfuerzos por mejorar sus métodos industriales. Envian un emisario a la Tierra que descubre la clave para esto es la refinación de petróleo y la libre empresa. Usaremos el vídeo para introducir algunas de las funcionalidades básicas de FFmpeg y  analizar sus propiedades de color en relación con su retórica propagandística.

{% include figure.html filename="destEarth_titlecard.png" caption="Destination Earth (1956)" %}

Para esta tutorial, necesitarás hacer:
...
* Guarde estos archivos en la carpeta misma en algún lugar de su computadora. Guárdalos con los nombres de archivos `destEarth` seguido de su extensión.
...
La línea `Input # 0` de el informe identifica el **contenedor** como [ogg](https://es.wikipedia.org/wiki/Ogg). Contenedores (también llamados "envoltorios") proporcionan el archivo con la estructura de sus diversas pistas. Contenedores diferentes (otros más comunes incluyen `.mkv`, `.avi`, y `.flv`) tienen diferentes características y compatibilidades con varios programas. Examinaremos cómo y por qué es posible que desee cambiar el contenedor de un archivo en el siguiente comando.

Las líneas `Stream #0:0` y `Stream #0:1` proporciona información sobre las pistas del archivo (esto es, el contenido que ve en la pantalla y escucha a través de sus altavoces) y identifica el **códec** de cada pista tambien. Los códecs especifican cómo se codifica/comprime la información (se escribe y almacena) y se decodifica (se reproduce) la información. La pista vídeo (`Stream #0:0`) de nuestro archivo `.ogv` usa el códec [theora](https://es.wikipedia.org/wiki/Theora) y la pista audio (`Stream #0:1`) usa el códec [vorbis](https://es.wikipedia.org/wiki/Vorbis). Estas líneas también proporcionan información importante relacionada con el espacio de color de la pista de vídeo (`yuv420p`), resolución (`400x300`), y marcos por segundo (`29.97 fps`). Adicionalmente, información de audio como la tasa de muestreo (`44100 Hz`) y la tasa de bits (`128 kb/s`).
...
Verá una ventana abierta, luego el vídeo comenzará, se reproducirá una vez y luego la ventana se cerrará (además, notarás que no hay sonido en su vídeo). También notará que los comandos `ffplay` no requieren que se especifique una entrada (`-i`) o una salida porque la reproducción en sí misma es la salida.
<div class="alert alert-warning">
...
FFmpeg se puede utilizar para reproducir y crear archivos de vídeo con vectorescopios incrustados en ellos para proporcionar una referencia en tiempo real para la información de color del vídeo. Los siguientes comandos `ffplay` incrustarán un vectorscopio en la esquina inferior derecha del marco. A medida que se reproduce el vídeo, notará el cambio de la trama de vectorscopio a medida que cambia el color en pantalla:
...
* `-vf` = crea un [*filter-graph*](https://urldefense.proofpoint.com/v2/url?u=https-3A__trac.ffmpeg.org_wiki_FilteringGuide&d=DwIGaQ&c=HPMtquzZjKY31rtkyGRFnQ&r=I2YgKB-JBNJOTO8vNfKgSdxt3vZ7dPAAM-jkyT9yS80&m=VcGpj8XU7DCg1rx1yOlHUQrjwXwrks1PDcZKJC0P-V0&s=mRlr8IA3sTGoboQ8yiPBQAg_Sie417PC4qiSJqPmb3A&e=) para usar con las pistas
...
En este tutorial, hemos aprendido:
  * Para instalar FFmpeg en diferentes sistemas operativos y cómo acceder al marco en el navegador web
  * La sintaxis básica y la estructura de los comandos FFmpeg
  * Para ver metadatos técnicos básicos de un archivo audiovisual
  * Para transformar un archivo audiovisual a través de la transcodificación y el reenvío
  * Para analizar y editar ese archivo audiovisual separando sus componentes ("demux") y creando extractos
  * Para reproducir archivos audiovisuales usando `ffplay`
  * Para crear nuevos archivos de vídeo con vectorescopios incrustados
  * Para exportar datos tabulares relacionados con el color de una pista de vídeo usando `ffprobe`
  * Para crear un en bucle de Bash para extraer información de datos de color de múltiples archivos de vídeo con un comando

En un nivel más amplio, este tutorial aspira a proporcionar una introducción informada y atractiva sobre cómo se pueden incorporar las herramientas y metodologías audiovisuales en los proyectos y prácticas de Humanidades Digitales. Con herramientas abiertas y potentes como FFmpeg, existe un gran potencial para expandir el alcance del campo para incluir tipos de medios y análisis más ricos y complejos que nunca.
